...
def _create_task(task_type, task_metadata, owner_id):
    if not isinstance(task_metadata, str):
...
def _start_background_task(app_context, task_id, claim):
    def start(task):
        task.status = "started"
        task.progress = 10
    ...
    def finish(task, result_message=None):
        task.status = "success"
        task.progress = 100
        task.result = (
    ...
    with app_context:
        ...
        try:
            ...
            elif task.task_type == "json_export":
                create_json(claim)
                finish(task, "JSON export completed successfully")
            elif task.task_type == "html_export":
                create_html(claim)
                finish(task, "HTML export completed successfully")
            elif task.task_type == "share_book_event":
...
def share_book(claim_id, data):
    ...
    if (not settings or not settings.mastodon_access_token or 
    ...
    try:
        ...
        if data.get("reading_status") == "Read":
